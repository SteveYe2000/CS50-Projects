...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("UPDATE users SET cash = cash - :total_cost WHERE id = :user_id", total_cost = total, user_id = session["user_id"])
...
def register():
    ...
    if request.method == "POST":
        ...
        r = db.execute("SELECT * FROM users WHERE username = ?", request.form.get("username"))
        if len(r) != 0:
